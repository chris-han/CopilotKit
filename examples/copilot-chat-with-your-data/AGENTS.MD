...
- `NEXT_PUBLIC_DATA_STORY_AUDIO_ENABLED` controls whether the client requests narrated audio for data stories (defaults to `true`); set it to `false` when the backend text-to-speech pipeline is unavailable.
